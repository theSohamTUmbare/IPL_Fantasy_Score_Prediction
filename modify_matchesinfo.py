...
if load:
    ...
    recent_matches_df = pd.read_csv('match_info.csv')
...
for i, match_id in enumerate(match_ids):
    ...
    if (len(match_data.json['match']['next_datetime_local'].split(" ")) == 1):
        data = match_data.json['match']['next_datetime_local'].split(" ")   
        if '-' in data:
            match_date = data[0]
            match_time = None
        else:
        ...
        match_date, match_time = match_data.json['match']['next_datetime_local'].split(" ") if match_data.json['match']['next_datetime_local'].split(" ") else (None, None)
...
recent_matches_df.to_csv('updated_match_info\\updated_matches_info1.csv', index=False)
exit()
